...
public class TDWorldGenerator implements IWorldGenerator
{
	...
	{
		addTowerGen(world, random, chunkX, chunkZ);
	...
	public void addTowerGen(World world, Random random, int chunkX, int chunkZ){
				...
				if(newX%(ConfigValues.towerRadius*2)==0 && newZ%(ConfigValues.towerRadius*2)==0){
					...
					for(int y=256; y>0; y--){
						if(world.getBlock(newX, y, newZ)==Blocks.air && world.getBlock(newX, y-1, newZ)!=Blocks.air){
							...
							TileEntityTower tower = (TileEntityTower) world.getTileEntity(newX, y, newZ);
							tower.orbs=-1;
